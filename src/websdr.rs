...
pub enum DemodMode { Wfm, Nfm, Am, Usb, Lsb, Off }
...
enum WebEvent {
    ...
    AudioConfig   { sample_rate: u32, fft_size: usize },
...
impl WebSdrServer {
    ...
    pub async fn start(
            ...
            tokio::spawn(async move {
                ...
                while let Some(res) = stream.next().await {
                    match res {
                        ...
                        Err(e)  => { error!("WebSDR pump: {:?}", e); break; }
                    }
                }
            });
        }
        ...
        let state = Arc::new(Self { driver, sample_tx, band, retune_tx, sample_rate: PIPELINE_SAMPLE_RATE });
    ...
    pub async fn start_shared(
        ...
        let state = Arc::new(Self { driver, sample_tx, band, retune_tx, sample_rate });
        Self::bind(state, addr, tls).await
    }

    async fn bind(state: Arc<Self>, addr: &str, tls: Option<(PathBuf, PathBuf)>) -> anyhow::Result<()> {
        let app = Router::new()
        ...
        if let Some((cert, key)) = tls {
            ...
            axum_server::bind_rustls(addr.parse()?, cfg).serve(app.into_make_service()).await?;
        } else {
...
async fn handle_socket(mut socket: WebSocket, state: Arc<WebSdrServer>) {
    ...
    let _ = socket.send(Message::Text(
        serde_json::to_string(&WebEvent::HardwareInfo {
            manufacturer: info.manufacturer, product: info.product, is_v4: info.is_v4,
        }).unwrap().into(),
    )).await;
    let _ = socket.send(Message::Text(
        serde_json::to_string(&WebEvent::AudioConfig {
            sample_rate: AUDIO_SAMPLE_RATE, fft_size: FFT_SIZE,
        }).unwrap().into(),
    )).await;
    let _ = socket.send(Message::Text(
        serde_json::json!({"type":"freqconfirm","requested":hw_freq,"actual":hw_freq}).to_string().into(),
    )).await;
    let _ = socket.send(Message::Text(
        serde_json::json!({"type":"gainconfirm","actual":gain}).to_string().into(),
    )).await;
    ...
    let mut pipeline_task  = tokio::spawn(run_client_pipeline(
        state.clone(), pipeline_sample_rx, demod_rx, bandwidth_rx, squelch_rx, freq_rx, pipeline_ws_tx, hw_freq,
    ));
    ...
    let mut cmd_task = tokio::spawn(async move {
        while let Some(Ok(Message::Text(text))) = receiver.next().await {
            let Ok(cmd) = serde_json::from_str::<Command>(&text) else { continue; };
            match cmd {
                Command::Frequency { hz } => {
                    if cmd_state.band.read().contains(hz) {
                        let _ = freq_tx.send(hz).await;
                    } else {
                        let _ = cmd_state.retune_tx.send(RetuneRequest { center_hz: hz }).await;
                        let _ = freq_tx.send(hz).await;
                    }
                    let reply = serde_json::json!({"type":"freqconfirm","requested":hz,"actual":hz});
                ...
                Command::Gain { db } => {
                    ...
                    let reply = match d.tuner.set_gain(db) {
                        ...
                        Err(e) => serde_json::json!({"type":"error","cmd":"setgain","msg":e.to_string()}),
                    };
                    let _ = cmd_reply_tx.send(Message::Text(reply.to_string().into())).await;
                }
                Command::Demod { mode } => {
                    let _ = demod_tx.send(mode);
                    let _ = cmd_reply_tx.send(Message::Text(
                        serde_json::json!({"type":"demodconfirm","mode":mode}).to_string().into()
                    )).await;
                }
                Command::Bandwidth { hz } => {
                    ...
                    let _ = cmd_reply_tx.send(Message::Text(
                        serde_json::json!({"type":"bandwidthconfirm","actual":clamped}).to_string().into()
                    )).await;
                }
                Command::Squelch { db } => {
                    let _ = squelch_tx.send(db);
                    let _ = cmd_reply_tx.send(Message::Text(
                        serde_json::json!({"type":"squelchconfirm","actual":db}).to_string().into()
                    )).await;
                }            }
        }
    });
...
impl DspState {
    pub fn new(sr: f32, fs: f64, initial_offset: f64) -> Self {
        Self {
            ...
            audio_agc: crate::dsp::AudioAgc::new(0.17, 0.1, 0.001, 500.0, AUDIO_SAMPLE_RATE as f32, 0.02),
    ...
    pub fn reset(&mut self) {
        self.pre_i_wfm.reset(); self.pre_q_wfm.reset(); self.post_wfm.reset();
        self.pre_i_nfm.reset(); self.pre_q_nfm.reset();
        self.pre_i_am.reset();  self.pre_q_am.reset();
        self.pre_i_ssb.reset(); self.pre_q_ssb.reset();
...
async fn run_client_pipeline(
    ...
    mut demod_rx:  broadcast::Receiver<DemodMode>,
    mut bw_rx:     broadcast::Receiver<u32>,
    mut squelch_rx: mpsc::Receiver<f32>,
    mut freq_rx:   mpsc::Receiver<u64>,
    ...
    loop {
        ...
        while let Ok(f) = freq_rx.try_recv()  { client_freq = f; }
        while let Ok(m) = demod_rx.try_recv() { mode = m; }
        while let Ok(sq) = squelch_rx.try_recv() { dsp.squelch_db = sq; }
        while let Ok(bw) = bw_rx.try_recv() {
            let norm = (bw as f32 / sr).clamp(0.001, 0.499);
            dsp.pre_i_ssb.update_cutoff(norm); dsp.pre_q_ssb.update_cutoff(norm);
            dsp.pre_i_am.update_cutoff(norm);  dsp.pre_q_am.update_cutoff(norm);
        }
        ...
        // ── Processing ────────────────────────────────────────────────────────
        if dsp.iq_buf.len() != n * 2 { dsp.iq_buf.resize(n * 2, 0.0); }
        crate::converter::convert(&raw, &mut dsp.iq_buf);
        dsp.nco.mix(&mut dsp.iq_buf);

        if dsp.i_buf.len() != n { dsp.i_buf.resize(n, 0.0); }
        if dsp.q_buf.len() != n { dsp.q_buf.resize(n, 0.0); }
        for k in 0..n { dsp.i_buf[k] = dsp.iq_buf[k*2]; dsp.q_buf[k] = dsp.iq_buf[k*2+1]; }
        if mode != DemodMode::Am { dsp.dc_iq.process_split(&mut dsp.i_buf, &mut dsp.q_buf); }
        ...
        if now.duration_since(last_frame) >= frame_iv && n >= FFT_SIZE {
            last_frame = now;
            let off = n - FFT_SIZE;
            for k in 0..FFT_SIZE { dsp.fft_buf[k] = Complex::new(dsp.i_buf[off+k], dsp.q_buf[off+k]); }
            ...
            avg_pwr  = if avg_pwr == 0.0 { ipwr } else { (1.0-ema)*avg_pwr + ema*ipwr };
            let mut mag = vec![0u8; FFT_SIZE];
            let inv = state.band.read().spectral_inv;
            for (i, m) in mag.iter_mut().enumerate() {
                let s = if inv { (FFT_SIZE - i + FFT_SIZE / 2) % FFT_SIZE } else { (i + FFT_SIZE / 2) % FFT_SIZE };
            ...
            wf.push(b'W'); wf.extend_from_slice(&mag);
        ...
        macro_rules! dec {
            ($pi:expr, $pq:expr) => {{
                ...
                for k in 0..len { dsp.iq_buf[k*2] = dsp.i_dec[k]; dsp.iq_buf[k*2+1] = dsp.q_dec[k]; }
                len
            }};
        }
        ...
        let mut audio: Vec<f32> = match mode {
            ...
            DemodMode::Nfm => { let len = dec!(dsp.pre_i_nfm, dsp.pre_q_nfm); dsp.fm_nfm.process(&dsp.iq_buf[..len*2]) }
            DemodMode::Am  => { let len = dec!(dsp.pre_i_am,  dsp.pre_q_am);  dsp.am.process(&dsp.iq_buf[..len*2]) }
            DemodMode::Usb => { let len = dec!(dsp.pre_i_ssb, dsp.pre_q_ssb); dsp.ssb_usb.process(&dsp.iq_buf[..len*2]) }
            DemodMode::Lsb => { let len = dec!(dsp.pre_i_ssb, dsp.pre_q_ssb); dsp.ssb_lsb.process(&dsp.iq_buf[..len*2]) }
        ...
        if !audio.is_empty() {
            ...
            let dbfs = if pwr > 1e-12 { 10.0 * pwr.log10() } else { -100.0 };
            ...
            for &s in &audio { au_msg.extend_from_slice(&s.to_le_bytes()); }
            if ws_tx.send(Message::Binary(au_msg.into())).await.is_err() { break; }
        }
        ...
        if client_freq != last_confirmed {
            ...
            if ws_tx.send(Message::Text(confirm.to_string().into())).await.is_err() { break; }

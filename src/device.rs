...
impl<T: UsbContext> Device<T> {
    ...
    pub fn read_info(&self) -> DeviceInfo { DeviceInfo::probe(&self.handle) }
}

impl<T: UsbContext> HardwareInterface for Device<T> {
    fn write_reg(&self, blk: u16, addr: u16, val: u8) -> Result<()> {
        let index = (blk << 8) | 0x10;
        self.handle.write_control(0x40, BREQUEST, addr, index, &[val], TIMEOUT)
            .map_err(|e| { error!("write_reg failed: {:?}", e); Error::Usb(e) })?;
        Ok(())
    }
    ...
    fn write_reg16(&self, blk: u16, addr: u16, val: u16) -> Result<()> {
        ...
        self.handle.write_control(0x40, BREQUEST, addr, index, &data, TIMEOUT)
            .map_err(|e| { error!("write_reg16 failed: {:?}", e); Error::Usb(e) })?;
        Ok(())
    }
    ...
    fn read_reg(&self, blk: u16, addr: u16) -> Result<u8> {
        let index = blk << 8;
        let mut buf = [0u8; 1];
        self.handle.read_control(0xc0, BREQUEST, addr, index, &mut buf, TIMEOUT)
            .map_err(|e| { error!("read_reg failed: {:?}", e); Error::Usb(e) })?;
        Ok(buf[0])
    }
    ...
    fn demod_write_reg(&self, page: u8, addr: u16, val: u8) -> Result<()> {
        ...
        self.handle.write_control(0x40, BREQUEST, addr_val, index, &[val], TIMEOUT)
            .map_err(|e| { error!("demod_write_reg(p={}, a=0x{:04x}) failed: {:?}", page, addr, e); Error::Usb(e) })?;
    ...
    fn demod_write_reg16(&self, page: u8, addr: u16, val: u16) -> Result<()> {
        ...
        let data = [(val >> 8) as u8, (val & 0xff) as u8];
        self.handle.write_control(0x40, BREQUEST, addr_val, index, &data, TIMEOUT)
            .map_err(|e| { error!("demod_write_reg16(p={}, a=0x{:04x}) failed: {:?}", page, addr, e); Error::Usb(e) })?;
        let _ = self.demod_read_reg(0x0a, 0x01);
        Ok(())
    }
    ...
    fn demod_read_reg(&self, page: u8, addr: u16) -> Result<u8> {
        ...
        let addr_val = (addr << 8) | 0x20;
        let mut buf = [0u8; 1];
        self.handle.read_control(0xc0, BREQUEST, addr_val, index, &mut buf, TIMEOUT)
            .map_err(|e| { error!("demod_read_reg(p={}, a=0x{:04x}) failed: {:?}", page, addr, e); Error::Usb(e) })?;
        Ok(buf[0])
    }
    ...
    fn i2c_write_raw(&self, addr: u8, data: &[u8]) -> Result<()> {
        ...
        self.handle.write_control(0x40, BREQUEST, addr as u16, index, data, TIMEOUT)
            .map_err(|e| { debug!("i2c_write_raw(addr=0x{:02x}) failed: {:?}", addr, e); Error::Usb(e) })?;
        Ok(())
    }
    ...
    fn i2c_read_raw(&self, addr: u8, len: usize) -> Result<Vec<u8>> {
        ...
        self.handle.read_control(0xc0, BREQUEST, addr as u16, index, &mut buf, TIMEOUT)
            .map_err(|e| { debug!("i2c_read_raw(addr=0x{:02x}) failed: {:?}", addr, e); Error::Usb(e) })?;
        Ok(buf)
    }
    ...
    fn read_bulk(&self, ep: u8, buf: &mut [u8], timeout: Duration) -> Result<usize> {
        Ok(self.handle.read_bulk(ep, buf, timeout).map_err(Error::Usb)?)
    }
...
impl Device<Context> {
    pub fn open() -> Result<Self> {
        ...
        for dev in devices.iter() {
            ...
            if desc.vendor_id() != 0x0bda || (desc.product_id() != 0x2838 && desc.product_id() != 0x2832) { continue; }
            let handle = dev.open()?;
            #[cfg(target_os = "linux")] { let _ = handle.set_auto_detach_kernel_driver(true); }
            handle.claim_interface(0)?;
            return Ok(Self { handle, _context: context });
        }
        Err(Error::NotFound)
    }
}
...
pub struct TransportBuffer<T: UsbContext> { data: Vec<u8>, _phantom: std::marker::PhantomData<T> }
impl<T: UsbContext> TransportBuffer<T> {
    pub fn new(_dev: Arc<Device<T>>, len: usize) -> Self { Self { data: vec![0u8; len], _phantom: std::marker::PhantomData } }
}
